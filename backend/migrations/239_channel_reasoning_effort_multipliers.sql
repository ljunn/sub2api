...
-- even when empty. Keep the legacy key for the production binary sharing this
-- database during preview. New JSON writes mirror max for rolling compatibility.
UPDATE groups AS g
SET model_pricing = (
    SELECT jsonb_agg(
        CASE
