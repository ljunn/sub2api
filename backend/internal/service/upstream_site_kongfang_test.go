...
func TestKongfangLifecycle(t *testing.T) {
	...
	require.Equal(t, defaultSiteConcurrency, siteAccountConcurrency(site), "unpublished concurrency defaults to 1000")
	...
	require.NoError(t, err)
	require.Equal(t, 1000, a.Concurrency, "new binding without a published cap uses the requested default")
