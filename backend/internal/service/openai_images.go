...
func (s *OpenAIGatewayService) ParseOpenAIImagesRequest(c *gin.Context, body []byte) (*OpenAIImagesRequest, error) {
	...
	if err := validateCompatibleImagesModel(req.Model); err != nil && !s.siteImageAliasAllowed(c.Request.Context(), req.Model) {
		return nil, err
	}
...
func (s *OpenAIGatewayService) forwardOpenAIImagesAPIKey(
	...
	if err := validateCompatibleImagesModel(requestModel); err != nil && !account.IsSiteManaged() {
		return nil, err
	}
	upstreamModel := account.GetMappedModel(requestModel)
	if err := validateCompatibleImagesModel(upstreamModel); err != nil && !account.IsSiteManaged() {
		return nil, err
	}
	...
	if resp.StatusCode >= 400 {
		...
		if s.shouldFailoverOpenAIUpstreamResponse(account, resp.StatusCode, upstreamMsg, respBody) {
			...
			if isOpenAIHTTPUpstreamAccessStateError(resp.StatusCode, upstreamMsg, respBody) || isOpenAIModelUnavailableResponse(resp.StatusCode, respBody) {
