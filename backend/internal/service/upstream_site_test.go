...
func TestUpstreamSiteLifecyclePriceChangeRecoveryAndStaleQueue(t *testing.T) {
	...
	assert.Empty(t, siteTierReason(policy, "1K", lastSuccess.Add(24*time.Hour)), "failed refreshes must not expire a known price")
	assert.Equal(t, "site_price_exceeded", siteTierReason(policy, "2K", lastSuccess.Add(24*time.Hour)))
