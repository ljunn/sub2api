...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (result *ForwardResult, err error) {
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		// 发送请求
		resp, err = siteCheckedUpstream(s.httpUpstream, s.accountRepo, account).DoWithTLS(upstreamReq, proxyURL, account.ID, account.Concurrency, tlsProfile)
		if err != nil {
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
					...
					releaseRetryCtx()
					if buildErr == nil {
						retryResp, retryErr := siteCheckedUpstream(s.httpUpstream, s.accountRepo, account).DoWithTLS(retryReq, proxyURL, account.ID, account.Concurrency, tlsProfile)
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isSignatureErrorPattern(ctx, account, retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									releaseRetryCtx2()
									if buildErr2 == nil {
										retryResp2, retryErr2 := siteCheckedUpstream(s.httpUpstream, s.accountRepo, account).DoWithTLS(retryReq2, proxyURL, account.ID, account.Concurrency, tlsProfile)
										if retryErr2 == nil {
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						if buildErr == nil {
							budgetRetryResp, retryErr := siteCheckedUpstream(s.httpUpstream, s.accountRepo, account).DoWithTLS(budgetRetryReq, proxyURL, account.ID, account.Concurrency, tlsProfile)
							if retryErr == nil {
