...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	if err := validateImageBackgroundExtra(input.Extra); err != nil {
		return nil, err
	}
	accountExtra, err := normalizeOpenAILongContextBillingExtra(input.Platform, input.Extra)
	if err != nil {
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Extra != nil {
		...
		// Older editors omit this setting. Preserve the explicit routing veto.
		if _, supplied := input.Extra[ImagesTransparentBackgroundSupportedExtraKey]; !supplied {
			if value, exists := account.Extra[ImagesTransparentBackgroundSupportedExtraKey]; exists {
				input.Extra[ImagesTransparentBackgroundSupportedExtraKey] = value
			}
		}
...
func (s *adminServiceImpl) UpdateAccountExtra(ctx context.Context, id int64, updates map[string]any) error {
	if err := validateImageBackgroundExtra(updates); err != nil {
		return err
	}
