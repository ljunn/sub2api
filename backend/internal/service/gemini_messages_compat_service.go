...
func (s *GeminiMessagesCompatService) SelectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*Account, error) {
	if !stickySessionsEnabled(s.cfg) {
		sessionHash = ""
	}
	// 1. 确定目标平台和调度模式
...
func (s *GeminiMessagesCompatService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (siteResult *ForwardResult, siteErr error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		resp, err = siteCheckedUpstream(s.httpUpstream, s.accountRepo, account).Do(upstreamReq, proxyURL, account.ID, account.Concurrency)
		if err != nil {
...
func (s *GeminiMessagesCompatService) ForwardAIStudioGET(ctx context.Context, account *Account, path string) (*UpstreamHTTPResult, error) {
	...
	resp, err := siteCheckedUpstream(s.httpUpstream, s.accountRepo, account).Do(req, proxyURL, account.ID, account.Concurrency)
	if err != nil {
		return nil, err
	}
	defer func() { _ = resp.Body.Close() }()
