...
type Group struct {
	...
	// 是否允许站点托管模型采购价等于本组采购上限；默认不允许
	AllowEqualPriceScheduling bool `json:"allow_equal_price_scheduling,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldAllowEqualPriceScheduling:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field allow_equal_price_scheduling", values[i])
			} else if value.Valid {
				_m.AllowEqualPriceScheduling = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("allow_equal_price_scheduling=")
	builder.WriteString(fmt.Sprintf("%v", _m.AllowEqualPriceScheduling))
	builder.WriteString(", ")
