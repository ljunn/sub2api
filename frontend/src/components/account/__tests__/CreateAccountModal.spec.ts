...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it('creates a LongXia account and keeps media protocols mutually exclusive', async () => {
    ...
    await selectButtonByText(wrapper, 'OpenAI')
    await selectButtonByText(wrapper, 'API Key')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('LongXia')
    await wrapper.get('form#create-account-form input[type="password"]').setValue('test-api-key')
    await wrapper.get('[data-testid="vividai-enabled"]').setValue(true)
    await wrapper.get('[data-testid="longxia-enabled"]').setValue(true)
    expect((wrapper.get('[data-testid="vividai-enabled"]').element as HTMLInputElement).checked).toBe(false)
    await wrapper.get('form#create-account-form').trigger('submit.prevent')
    await flushPromises()
    expect(createAccountMock.mock.calls[0]?.[0]?.extra?.longxia_enabled).toBe(true)
    expect(createAccountMock.mock.calls[0]?.[0]?.extra?.vividai_enabled).toBeUndefined()
    wrapper.unmount()
  })
