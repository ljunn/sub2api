<template>
  <section class="space-y-3" data-testid="site-overview">
    ...
    <div class="lg:max-h-[65vh] lg:overflow-auto lg:rounded-xl lg:border lg:border-gray-200 lg:bg-white lg:dark:border-dark-700 lg:dark:bg-dark-800">
      <table class="block w-full text-left text-sm lg:table">
        <thead class="sticky top-0 z-10 hidden lg:table-header-group bg-gray-50 text-xs text-gray-500 dark:bg-dark-900"><tr>
        ...
        <tbody class="grid gap-3 sm:grid-cols-2 lg:table-row-group lg:divide-y lg:divide-gray-100 lg:dark:divide-dark-700">
          <tr v-for="site in filteredSites" :key="site.id" :data-site="site.id"
            tabindex="0" :aria-current="selectedId === site.id ? 'true' : undefined"
            class="grid min-w-0 cursor-pointer grid-cols-2 gap-3 rounded-xl border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-800 lg:table-row lg:rounded-none lg:border-0 lg:bg-transparent lg:p-0 lg:dark:bg-transparent outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary-500"
            ...
            <td class="col-span-2 min-w-0 lg:px-4 lg:py-2">
              <button type="button" class="block min-h-9 text-left font-semibold text-primary-700 [overflow-wrap:anywhere] dark:text-primary-400 lg:min-h-0 lg:max-w-56 lg:truncate" :title="site.name" @click.stop="$emit('select', site.id)">{{ site.name }}</button>
              <div class="mt-0.5 text-xs text-gray-500 [overflow-wrap:anywhere] lg:max-w-56 lg:truncate" :title="site.base_url">{{ site.kind === 'wuzu' ? 'WUZU' : site.kind === 'vividai' ? 'VividAI' : site.kind === 'kongfang' ? t('admin.sites.kongfang') : site.kind === 'newapi' ? 'New API' : 'Sub2API' }} · {{ site.base_url }}</div>
              <a :href="site.base_url" target="_blank" rel="noopener noreferrer"
                class="mt-1 inline-flex min-h-9 items-center gap-1 text-xs text-primary-600 hover:underline lg:min-h-0"
            ...
            <td class="min-w-0 lg:whitespace-nowrap lg:px-4 lg:py-2">
              <span class="mb-1 block text-xs text-gray-500 lg:hidden">{{ t('admin.sites.overview.updates') }}</span>
              <button v-if="unreadCount(site)" type="button" class="min-h-9 rounded-full bg-blue-50 px-2 py-1 lg:min-h-0 text-xs font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-300" @click.stop="$emit('models', site.id)">{{ t('admin.sites.overview.newCount', { count: unreadCount(site) }) }}</button>
              <span v-else class="text-xs text-gray-400">{{ t('admin.sites.overview.noNew') }}</span>
            </td>
            <td class="min-w-0 lg:whitespace-nowrap lg:px-4 lg:py-2">
              <span class="mb-1 block text-xs text-gray-500 lg:hidden">{{ t('admin.siteBalance.balance') }}</span>
              <button type="button" class="min-h-9 text-left [overflow-wrap:anywhere] lg:min-h-0" :title="site.balance?.error || site.balance?.last_success" @click.stop="$emit('select', site.id)">
            ...
            <td class="min-w-0 text-xs [overflow-wrap:anywhere] lg:px-4 lg:py-2" :title="site.error || site.balance?.error">
              <span class="mb-1 block text-gray-500 lg:hidden">{{ t('admin.sites.overview.connection') }}</span>
            ...
            <td class="min-w-0 text-gray-500 lg:px-4 lg:py-2 lg:text-right"><span class="mb-1 block text-xs lg:hidden">{{ t('admin.sites.bindings') }}</span>{{ site.bindings.length }}</td>
          </tr>
