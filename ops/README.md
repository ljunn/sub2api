...
2026-09-21 后续整合新增：多站点总览直接展示余额和新增模型数，支持异常筛选与搜索；上游新增模型按管理员保存已读状态，实际查看后清除未读提示，绑定仍由管理员决定。后续审阅修复同时包含站点整行切换与异步结果不抢选择、列表主页面与站点详情弹窗，以及飞羽公开模型价格接口兼容。这些功能与上述余额、空凡、价格调度、紧凑布局及命名修改合入同一个完整 HEAD，详见 [UPSTREAM_SITES.md](UPSTREAM_SITES.md)。
