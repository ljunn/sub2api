...
视频按每秒积分 × 最长支持时长 × 积分美元成本形成各清晰度的保守采购价。当前 seedance-2.0 为 720p、固定 15 秒、14 积分/秒。支持本地明确的按次价或输出积分等价单位价；不使用默认原生 Ark token 单价。按单位计费仍为 1 积分 = 1000 输出单位，例如每积分售价 $0.002 对应每百万输出单位 $2。视频不使用普通图片/token 手动采购价覆盖；新视频缺少完整时长或规格时阻断。已受理任务续查不因调价/价格过期再次下单或被价格检查拦截，所有权和分组校验保留。
