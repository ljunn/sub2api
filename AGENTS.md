...
- 保留已有未提交修改，不得 reset 或覆盖。用户已明确要求 Sub2API 的全部待发布改动一起提交、推送和打包；发布范围包含已有未提交的前端、后端、配置、测试和文档，不得仅挑选最近一个功能。凭据、运行配置和临时产物仍不得入库。
- 预览和生产构建前必须完成全部待发布改动的检查与提交，工作区保持干净。不得用独立索引、部分暂存或 `--committed-head` 等方式跳过剩余改动而发布功能不完整的预览。发现新增或并行改动时，先完成统一整合、测试和提交，再重新构建。
- 6556 必须运行包含全部待发布功能的统一版本；告知用户本次包含的功能。生产发布只能使用用户已审阅并明确确认的完整版本。
- 本机维护和生产分支统一为 `main`，跟踪本机维护仓库 `origin/main`，不得跟踪 `upstream/main`。`host-production` 仅保留为迁移前的历史分支，不再作为发布入口。
- 生产代码先合入并推送至 `origin/main`，再从本机已提交 HEAD 构建。构建前后及部署前，HEAD、本地 `main` 和现场获取的 `origin/main` 必须完全一致且工作区干净；允许固定在同一 main 提交的 detached checkout。不得绕过 `ops/require-main.sh`。
- 任务结束时固定源码目录保持干净、同步的 `main`。Git 提交信息使用中文，保留必要技术标识。
